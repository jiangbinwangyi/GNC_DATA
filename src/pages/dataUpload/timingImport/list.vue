<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.titName" placeholder="用户名" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
      ...
      <el-button class="filter-item add-button" type="primary" icon="el-icon-plus" @click="enterImport">
        新建任务
      </el-button>
    </div>
    ...
    >
      <el-table-column label="任务名称" prop="titName" align="center" min-width="80">
        <template slot-scope="{row}">
          <span>{{ row.titName }}</span>
        </template>
      </el-table-column>
        ...
        <template slot-scope="{row,$index}">
          ...
          <el-dropdown trigger="click" style="padding-left:10px" @command="handleMore($event, row, $index)">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item command="handleDelete">查看表</el-dropdown-item>
              <el-dropdown-item command="handleResetPasswd">查看型号</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="110px" style="width: 400px; margin-left:50px;">
        <el-form-item label="名称" prop="titName">
...
<script>
import { fetchList } from '@/api/dataUpload/timingImport'
...
export default {
  ...
  data() {
    return {
      ...
      pvData: [],
      rules: {
        titName: [{ required: true, message: '用户名必填', trigger: 'blur' }],
