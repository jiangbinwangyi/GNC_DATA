<template>
  <div>
    ...
    >
      ...
      <el-table-column label="测试编号名称" prop="satelliteName" align="center" min-width="120">
        <template slot-scope="{row}">
...
<script>
import { fetchList, deleteArticle, createArticle, updateArticle } from '@/api/metaManage/testProject'
...
export default {
  ...
  mounted() {
    this.listQuery.sateInfoId = this.$route.params.id
    this.getList()
  },
  methods: {
    toTable(row) {
      ...
      this.$router.push({ path: '/metaManage/index/table/' + tpiId, query: { type: 'tpi' }})
    },
    getList() {
